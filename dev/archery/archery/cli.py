...
@archery.group()
@click.pass_context
def bundled_thirdparty(ctx):
    """ Arrow thirdparty bundle management.
    """
    pass

bundled_thirdparty._versions_cmake_headmatter = '''\
# Licensed to the Apache Software Foundation (ASF) under one
# or more contributor license agreements.  See the NOTICE file
# distributed with this work for additional information
# regarding copyright ownership.  The ASF licenses this file
...
bundled_thirdparty._versions_cmake_vars = lambda name, bundle: f'''
if(DEFINED ENV{{ARROW_{name}_URL}})
  set({name}_SOURCE_URL "$ENV{{ARROW_{name}_URL}}")
else()
  set_urls(
    {name}_SOURCE_URL
    {
    """
    """.join(map(lambda url: f'"{url}"', bundle.urls))
    })
endif()

set(ARROW_{name}_BUILD_VERSION "{bundle.version}")

set({name}_BUILD_MD5_CHECKSUM "MD5={
  bundle.trimmed_or_empty["md5"] or bundle.full["md5"]
}")

'''

def bundled_thirdparty_yaml(cmd):
    ...
    class Version(str):
        ...
        @classmethod
        def to_yaml(cls, representer, version):
            return representer.represent_scalar(cls.yaml_tag, version)

        @property
        def dots_as_underscores(self):
    ...
    class Bundle:
        ...
        @property
        def name(self):
            return self._format(self.name_template)

        @property
        def canonical_url(self):
            for template in self.full['url_templates']:
                if 'https://github.com/ursa-labs/thirdparty' in template:
                    continue
                if 'https://dl.bintray.com/ursalabs' in template:
                    continue
                return self._format(template)

            raise Exception("No canonical url specified!")

        @property
        def trimmed_or_empty(self):
            empty = dict(md5=None, url_templates=[], script=None)
            return getattr(self, 'trimmed', empty)

        @property
        def urls(self):
            trimmed = map(self._format, self.trimmed_or_empty['url_templates'])
            full = map(self._format, self.full['url_templates'])
            return  [*trimmed, *full]

    return partial(cmd, yaml=yaml)


def bundled_thirdparty_common_options(cmd):
    options = [
        click.option("--src", metavar="<arrow_src>", show_default=True,
                     default=ArrowSources.find(),
                     callback=validate_arrow_sources,
...
@bundled_thirdparty.command(name="versions",
                            short_help="Manage bundle versions.")
@bundled_thirdparty_common_options
@click.option('--update', type=str, multiple=True,
              help='Set version for one or more bundles, for example '
                   '--update=AWSSDK@1.7.200 --update=GLOG@0.4.0 '
                   '(This does not update checksums or mirrors!)')
@click.pass_context
@bundled_thirdparty_yaml
def bundled_thirdparty_versions(ctx, src, update, yaml):

    with open(src / 'cpp/thirdparty/versions.yml') as stream:
        bundles = yaml.load(stream)

    if len(update) == 0:
        # print bundles and their current versions
        for name, bundle in bundles.items():
            click.echo(f'{name}@{bundle.version}')
        return

    for name, new_version in map(lambda s: s.split('@'), update):
        bundles[name].version = new_version

    with open(src / 'cpp/thirdparty/versions.yml', 'w') as stream:
        yaml.dump(bundles, stream)
        # also regenerate versions.cmake

    with open(src / 'cpp/thirdparty/versions.cmake', 'w') as stream:
        stream.write(bundled_thirdparty._versions_cmake_headmatter)

        for name, bundle in bundles.items():
            stream.write(bundled_thirdparty._versions_cmake_vars(name, bundle))

    try:
        linter(src, fix=True, iwyu_all=False, cmake_format=True)
    except LintValidationException:
        sys.exit(1)


@bundled_thirdparty.command(name="download", short_help="Download bundles.")
@bundled_thirdparty_common_options
@click.option('--canonical-only', is_flag=True, default=False,
              help='Download only official bundles.')
@click.option('--destination', default=os.getcwd(),
              type=click.Path(dir_okay=True, file_okay=False,
                              resolve_path=True),
              help='Download only official bundles.')
@click.option('--dry-run', is_flag=True, default=False,
              help='Print a summary instead of downloading.')
@click.option('--env-setters-only', is_flag=True, default=False,
              help='Write environment setter script instead of downloading.')
@click.pass_context
@bundled_thirdparty_yaml
def bundled_thirdparty_download(ctx, src, canonical_only, dry_run,
                                destination, env_setters_only, yaml):
    with open(src / 'cpp/thirdparty/versions.yml') as stream:
        bundles = yaml.load(stream)
    ...
    if dry_run:
        assert not env_setters_only

        click.echo('without --dry-run bundles would download into: '
                   f'{destination}')
        for name, bundle in bundles.items():
            click.echo(f"\n{name}:")
            click.echo(f'  from url: {url(bundle)}')
            click.echo(f'  basename: {bundle.name}')
        return

    _mkdirp(destination)

    click.echo(f'writing env setters to: {destination}/env.sh')
    with open(f'{destination}/env.sh', 'w') as env_setters:
        for name, bundle in bundles.items():
            local = f'{destination}/{bundle.name}'
            env_setters.write(f"export ARROW_{name}_URL={local}\n")
    ...
    for name, bundle in bundles.items():
        response = requests.get(url(bundle))
        local = f'{destination}/{bundle.name}'
        with open(local, 'wb') as stream:
            stream.write(response.content)


@bundled_thirdparty.command(name="upload",
                            short_help='Upload bundles to mirrors '
                                       'and update checksums.')
@click.option('--local-archives', default=os.getcwd(),
              type=click.Path(dir_okay=True, file_okay=False,
                              resolve_path=True),
              required=True,
              help='Directory containing downloaded canonical bundles.')
@click.option('--no-trimming', is_flag=True, default=False,
              help='Do not generate trimmed archives.')
@click.option('--update-checksums-only', is_flag=True, default=False,
              help='Update checksums only; no uploading.')
@bundled_thirdparty_common_options
@click.pass_context
@bundled_thirdparty_yaml
def bundled_thirdparty_upload(ctx, src, local_archives, no_trimming,
                              update_checksums_only, yaml):
    import hashlib
    ...
    for name, bundle in bundles.items():
        local = f'{local_archives}/{bundle.name}'
        md5 = hashlib.md5()
        with open(local, 'rb') as stream:
            md5.update(stream.read())
        bundles[name].full['md5'] = md5.hexdigest()

    if not no_trimming:
        for name, bundle in bundles.items():
            if bundle.trimmed_or_empty.script is None:
                continue

            import tarfile
            # decompress bundle into a tempdir and run the script
    ...
    raise NotImplementedError('TODO(bkietz) upload')
