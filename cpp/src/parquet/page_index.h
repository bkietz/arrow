...
/// ColumnIndex is a proxy around format::ColumnIndex.
class PARQUET_EXPORT ColumnIndex {
 public:
  /// Create a ColumnIndex from a serialized thrift message.
  ...
  /// A bitmap with a bit set for each data page that has only null values.
  ///
  ...
  /// A vector of encoded lower bounds for each data page in this column.
  ///
  ...
  /// A vector of encoded upper bounds for each data page in this column.
  ///
  /// `null_pages` should be inspected first, as only pages with non-null values
  /// may have their upper bounds populated.
  virtual const std::vector<std::string>& encoded_max_values() const = 0;

  /// The ordering of lower and upper bounds.
  ///
  ...
  /// Whether per-page null count information is available.
  virtual bool has_null_counts() const = 0;

  /// An optional vector with the number of null values in each data page.
  ///
  ...
  /// A vector of page indices for non-null pages.
  virtual const std::vector<int32_t>& non_null_page_indices() const = 0;
};

/// Typed implementation of ColumnIndex.
template <typename DType>
class PARQUET_EXPORT TypedColumnIndex : public ColumnIndex {
 public:
  using T = typename DType::c_type;

  /// A vector of lower bounds for each data page in this column.
  ///
  ...
  /// A vector of upper bounds for each data page in this column.
  ///
...
/// PageLocation is a proxy around format::PageLocation.
...
/// OffsetIndex is a proxy around format::OffsetIndex.
class PARQUET_EXPORT OffsetIndex {
 public:
  /// Create a OffsetIndex from a serialized thrift message.
  ...
  /// A vector of locations for each data page in this column.
  virtual const std::vector<PageLocation>& page_locations() const = 0;
};

/// Interface for reading the page index for a Parquet row group.
class PARQUET_EXPORT RowGroupPageIndexReader {
 public:
  ...
  /// :return: column index of the column or nullptr if it does not exist.
  ...
  ///
  /// :param i: column ordinal of the column chunk.
  /// :return: offset index of the column or nullptr if it does not exist.
  /// :throws ParquetException: if the index is out of bound.
...
/// Interface for reading the page index for a Parquet file.
class PARQUET_EXPORT PageIndexReader {
 public:
  virtual ~PageIndexReader() = default;

  /// Create a PageIndexReader instance.
  /// :return: a PageIndexReader instance.
  ...
  /// Get the page index reader of a specific row group.
  /// :param i: row group ordinal to get page index reader.
  /// :return: RowGroupPageIndexReader of the specified row group. A nullptr may or may
  ...
  /// Advise the reader which part of page index will be read later.
  ///
  ...
  ///    to access.
  ///
  /// :param row_group_indices: list of row group ordinal to read page index later.
  /// :param column_indices: list of column ordinal to read page index later. If it is
  ///            empty, it means all columns in the row group will be read.
  /// :param selection: which kind of page index is required later.
  virtual void WillNeed(const std::vector<int32_t>& row_group_indices,
                        const std::vector<int32_t>& column_indices,
                        const PageIndexSelection& selection) = 0;

  /// Advise the reader page index of these row groups will not be read anymore.
  ///
  ...
  ///
  /// :param row_group_indices: list of row group ordinal that whose page index will
  /// not be accessed anymore.
  virtual void WillNotNeed(const std::vector<int32_t>& row_group_indices) = 0;

  /// Determine the column index and offset index ranges for the given row group.
  ///
  /// :param row_group_metadata: row group metadata to get column chunk metadata.
  /// :param columns: list of column ordinals to get page index. If the list is empty,
  ///            it means all columns in the row group.
  /// :return: RowGroupIndexReadRange of the specified row group. Throws ParquetException
...
class PARQUET_EXPORT ColumnIndexBuilder {
 public:
  ...
  /// Serialize the column index thrift message.
  ///
  /// If the ColumnIndexBuilder has seen any corrupted statistics, it will
  /// not write any data to the sink.
  ///
  /// :param sink[out]: output stream to write the serialized message.
  /// :param encryptor: encryptor to encrypt the serialized column index.
  virtual void WriteTo(::arrow::io::OutputStream* sink,
                       Encryptor* encryptor = NULLPTR) const = 0;

  /// Create a ColumnIndex directly.
  ///
...
/// Interface for collecting offset index of data pages in a column chunk.
class PARQUET_EXPORT OffsetIndexBuilder {
 public:
  /// API convenience to create a OffsetIndexBuilder.
  static std::unique_ptr<OffsetIndexBuilder> Make();

  virtual ~OffsetIndexBuilder() = default;

  /// Add page location of a data page.
  ...
  /// Serialize the offset index thrift message.
  ///
  /// :param sink[out]: output stream to write the serialized message.
  /// :param encryptor: encryptor to encrypt the serialized offset index.
  virtual void WriteTo(::arrow::io::OutputStream* sink,
                       Encryptor* encryptor = NULLPTR) const = 0;

  /// Create an OffsetIndex directly.
  virtual std::unique_ptr<OffsetIndex> Build() const = 0;
};

/// Interface for collecting page index of a parquet file.
class PARQUET_EXPORT PageIndexBuilder {
 public:
  /// API convenience to create a PageIndexBuilder.
  ...
  /// Get the ColumnIndexBuilder from column ordinal.
  ///
  ...
  /// Get the OffsetIndexBuilder from column ordinal.
  ///
  /// :param i: Column ordinal.
  ...
  /// Complete the page index builder and no more write is allowed.
  virtual void Finish() = 0;

  /// Serialize the page index thrift message.
  ///
  /// Only valid column indexes and offset indexes are serialized and their locations
  /// are set.
  ///
  /// :param sink[out]: The output stream to write the page index.
  /// :param location[out]: The location of all page index to the start of sink.
  virtual void WriteTo(::arrow::io::OutputStream* sink,
