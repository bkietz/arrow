...
class ARROW_EXPORT IntegrationJsonWriter {
 public:
  ...
  ///
  /// :param schema: the schema of record batches
...
class ARROW_EXPORT IntegrationJsonReader {
 public:
  ...
  /// :param data: a Buffer containing the JSON data
  ...
  ///
  /// :param pool: a MemoryPool to use for buffer allocations
  /// :param in_file: a ReadableFile containing JSON data
  /// :return: the created JSON reader
  static Result<std::unique_ptr<IntegrationJsonReader>> Open(
  ...
  ///
  /// :param i: the record batch index, does not boundscheck
