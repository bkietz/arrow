...
class PartitionLocks {
 public:
  PartitionLocks();
  ~PartitionLocks();
  /// Initializes the control, must be called before use
  ///
  ...
  /// Cleans up the control, it should not be used after this call
  void CleanUp();
  /// Acquire a partition to work on one
  ///
  ...
  /// :param locked_prtn_id[out]: The id of the partition locked
  /// :param locked_prtn_id_pos[out]: The index of the partition locked in prtns_to_try
  ...
  /// Release a partition so that other threads can work on it
