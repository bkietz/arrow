...
class ARROW_ACERO_EXPORT SchemaSourceNodeOptions : public ExecNodeOptions {
 public:
  ...
  /// If true then items will be fetched from the iterator on a dedicated I/O
...
/// a node which executes expressions on input batches, producing batches
...
class ARROW_ACERO_EXPORT AsofJoinNodeOptions : public ExecNodeOptions {
 public:
  ...
  /// AsofJoin keys per input table. At least two keys must be given. The first key
  ...
  /// Tolerance for inexact "on" key matching. A right row is considered a match
