...
/// Preallocate the physical values array for a run-end encoded array
///
...
///
/// :postcondition: if has_validity_buffer and length > 0, then data.buffer[0] != NULLPTR
///
...
/// Preallocate the ArrayData for the run-end encoded version
...
///
/// :postcondition: if has_validity_buffer and physical_length > 0, then
...
/// Writes a single run-end to the first slot of the pre-allocated
