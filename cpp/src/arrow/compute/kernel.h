...
/// Base class for opaque kernel-specific state. For example, if there
/// is some kind of initialization required.
struct ARROW_EXPORT KernelState {
  virtual ~KernelState() = default;
};

/// Context/state for the execution of a particular kernel.
class ARROW_EXPORT KernelContext {
 public:
  ...
  /// Allocate buffer from the context's memory pool. The contents are
  /// not initialized.
  Result<std::shared_ptr<ResizableBuffer>> Allocate(int64_t nbytes);

  /// Allocate buffer for bitmap from the context's memory pool. Like
  ...
  /// Assign the active KernelState to be utilized for each stage of
  ...
  /// Configuration related to function execution that is to be shared
  /// across multiple kernels.
  ExecContext* exec_context() { return exec_ctx_; }

  /// The memory pool to use for allocations. For now, it uses the
...
/// An type-checking interface to permit customizable validation rules
...
struct ARROW_EXPORT TypeMatcher {
  ...
  /// Return true if this matcher accepts the data type.
  virtual bool Matches(const DataType& type) const = 0;

  /// A human-interpretable string representation of what the type
  /// matcher checks for, usable when printing KernelSignature or formatting
  /// error messages.
  virtual std::string ToString() const = 0;

  /// Return true if this TypeMatcher contains the same matching rule as
...
/// Match any DataType instance having the same DataType::id.
ARROW_EXPORT std::shared_ptr<TypeMatcher> SameTypeId(Type::type type_id);

/// Match any TimestampType instance having the same unit, but the time
...
/// Match run-end encoded types that use any valid run-end type and
/// encode specific value types
///
/// :param value_type_matcher: a matcher that is applied to the values field
...
/// encode specific value types
///
/// :param value_type_id: a type id that the type of the values field should match
ARROW_EXPORT std::shared_ptr<TypeMatcher> RunEndEncoded(Type::type value_type_id);

/// Match run-end encoded types that encode specific run-end and value types
///
/// :param run_end_type_matcher: a matcher that is applied to the run_ends field
/// :param value_type_matcher: a matcher that is applied to the values field
ARROW_EXPORT std::shared_ptr<TypeMatcher> RunEndEncoded(
...
/// An object used for type-checking arguments to be passed to a kernel
...
class ARROW_EXPORT InputType {
 public:
  /// The kind of type-checking rule that the InputType contains.
  enum Kind {
    /// Accept any value type.
    ANY_TYPE,

    /// A fixed arrow::DataType and will only exact match having this
    /// exact type (e.g. same TimestampType unit, same decimal scale and
    /// precision, or same nested child types).
    EXACT_TYPE,

    /// Uses a TypeMatcher implementation to check the type.
    USE_TYPE_MATCHER
  };
  ...
  /// Match any type with the given Type::type. Uses a TypeMatcher for
  /// its implementation.
  ...
  /// Return true if this input type matches the same type cases as the
  /// other.
  ...
  /// Render a human-readable string representation.
  std::string ToString() const;

  /// Return true if the Datum matches this argument kind in
  /// type (and only allows scalar or array-like Datums).
  bool Matches(const Datum& value) const;

  /// Return true if the type matches this InputType
  bool Matches(const DataType& type) const;

  /// The type matching rule that this InputType uses.
  Kind kind() const { return kind_; }

  /// For InputType::EXACT_TYPE kind, the exact type that this InputType
  /// must match. Otherwise this function should not be used and will assert in
  /// debug builds.
  const std::shared_ptr<DataType>& type() const;

  /// For InputType::USE_TYPE_MATCHER, the TypeMatcher to be used for
...
/// Container to capture both exact and input-dependent output types.
class ARROW_EXPORT OutputType {
 public:
  /// An enum indicating whether the value type is an invariant fixed
  ...
  /// Return the type of the expected output value of the kernel given
  ...
  /// The exact output value type for the FIXED kind.
  const std::shared_ptr<DataType>& type() const;

  /// For use with COMPUTED resolution strategy. It may be more
  ...
  /// Return the kind of type resolution of this output type, whether
...
/// Holds the input types and output type of the kernel.
///
...
class ARROW_EXPORT KernelSignature {
 public:
  ...
  /// Convenience ctor since make_shared can be awkward
  static std::shared_ptr<KernelSignature> Make(std::vector<InputType> in_types,
                                               OutputType out_type,
                                               bool is_varargs = false);

  /// Return true if the signature if compatible with the list of input
  /// value descriptors.
  bool MatchesInputs(const std::vector<TypeHolder>& types) const;

  /// Returns true if the input types of each signature are
  ...
  /// Compute a hash code for the signature
  size_t Hash() const;

  /// The input types for the kernel. For VarArgs functions, this should
  ...
  /// The output type for the kernel. Use Resolve to return the
...
/// A function may contain multiple variants of a kernel for a given
...
/// The strategy to use for propagating or otherwise populating the
...
/// The preference for memory preallocation of fixed-width type outputs
/// in kernel execution.
...
/// Arguments to pass to an KernelInit function. A struct is used to help
/// avoid API breakage should the arguments passed need to be expanded.
struct KernelInitArgs {
  /// A pointer to the kernel being initialized. The init function may
  /// depend on the kernel's KernelSignature or other data contained there.
  const Kernel* kernel;

  /// The types of the input arguments that the kernel is
  /// about to be executed against.
  const std::vector<TypeHolder>& inputs;

  /// Opaque options specific to this kernel. May be nullptr for functions
  /// that do not require options.
  const FunctionOptions* options;
};

/// Common initializer function for all kernel types.
using KernelInit = std::function<Result<std::unique_ptr<KernelState>>(
    KernelContext*, const KernelInitArgs&)>;

/// Base type for kernels. Contains the function signature and
...
struct ARROW_EXPORT Kernel {
  ...
  /// The "signature" of the kernel containing the InputType input
  /// argument validators and OutputType output type resolver.
  std::shared_ptr<KernelSignature> signature;

  /// Create a new KernelState for invocations of this kernel, e.g. to
  /// set up any options or state relevant for execution.
  KernelInit init;

  /// Create a vector of new KernelState for invocations of this kernel.
  static Status InitAll(KernelContext*, const KernelInitArgs&,
                        std::vector<std::unique_ptr<KernelState>>*);

  /// Indicates whether execution can benefit from parallelization
  ...
  /// Indicates the level of SIMD instruction support in the host CPU is
...
/// The scalar kernel execution API that must be implemented for SCALAR
...
/// Kernel data structure for implementations of ScalarFunction. In
...
struct ARROW_EXPORT ScalarKernel : public Kernel {
  ...
  /// Perform a single invocation of this kernel. Depending on the
  ...
  /// Writing execution results into larger contiguous allocations
...
/// Kernel data structure for implementations of VectorFunction. In
...
struct ARROW_EXPORT VectorKernel : public Kernel {
  /// See VectorKernel::finalize member for usage
  using FinalizeFunc = std::function<Status(KernelContext*, std::vector<Datum>*)>;

  /// Function for executing a stateful VectorKernel against a
  ...
  /// Perform a single invocation of this kernel. Any required state is
  /// managed through the KernelContext.
  ArrayKernelExec exec;

  /// Execute the kernel on a ChunkedArray. Does not need to be defined
  ChunkedExec exec_chunked = NULLPTR;

  /// For VectorKernel, convert intermediate results into finalized
...
/// Kernel data structure for implementations of
...
struct ARROW_EXPORT ScalarAggregateKernel : public Kernel {
  ...
  /// Merge a vector of KernelStates into a single KernelState.
  ...
  /// Whether this kernel requires ordering
...
struct ARROW_EXPORT HashAggregateKernel : public Kernel {
  ...
  /// whether the summarizer requires ordering
