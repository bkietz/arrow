...
class ARROW_EXPORT Table {
 public:
  ...
  ///
  /// :param schema: the schema of the empty Table
  ...
  ///
  /// :param reader: the arrow::RecordBatchReader that produces batches
  static Result<std::shared_ptr<Table>> FromRecordBatchReader(RecordBatchReader* reader);

  /// Construct a Table from RecordBatches, using schema supplied by the first
  /// RecordBatch.
  ///
  /// :param batches: a std::vector of record batches
  static Result<std::shared_ptr<Table>> FromRecordBatches(
      const std::vector<std::shared_ptr<RecordBatch>>& batches);

  /// Construct a Table from RecordBatches, using supplied schema. There may be
  /// zero record batches
  ///
  /// :param schema: the arrow::Schema for each batch
  /// :param batches: a std::vector of record batches
  static Result<std::shared_ptr<Table>> FromRecordBatches(
      std::shared_ptr<Schema> schema,
      const std::vector<std::shared_ptr<RecordBatch>>& batches);

  /// Construct a Table from a chunked StructArray. One column will be produced
  /// for each field of the StructArray.
  ///
  /// :param array: a chunked StructArray
  ...
  /// Construct a zero-copy slice of the table with the
  /// indicated offset and length
  ///
  /// :param offset: the index of the first row in the constructed
  /// slice
  /// :param length: the number of rows of the slice. If there are not enough
  ...
  /// Flatten the table, producing a new Table.  Any column with a
  ...
  ///
  /// :param pool: The pool for buffer allocations
...
/// Controls the behavior of ConcatenateTables().
...
///
/// :param tables: a std::vector of Tables to be concatenated
/// :param options: specify how to unify schema of input tables
/// :param memory_pool: MemoryPool to be used if null-filled arrays need to
...
/// :param schema: the target schema to promote to
/// :param pool: The memory pool to be used if null-filled arrays need to
/// be created.
ARROW_EXPORT
...
/// :param table: the input Table
/// :param schema: the target schema to promote to
/// :param options: The cast options to allow promotion of types
/// :param pool: The memory pool to be used if null-filled arrays need to
/// be created.
ARROW_EXPORT
Result<std::shared_ptr<Table>> PromoteTableToSchema(
