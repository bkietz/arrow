...
/// Object containing a pointer to a piece of contiguous memory with a
/// particular size.
///
...
class ARROW_EXPORT Buffer {
 public:
  ...
  ///
  /// :param data: a string_view object
  ///
  /// ```{note}
  /// The memory viewed by data must not be deallocated in the lifetime of the
  ...
  /// Construct an immutable buffer that takes ownership of the contents
  /// of an std::string (without copying it).
  ///
  /// :param data: a string to own
  ...
  /// Create buffer referencing typed memory with some length without
  /// copying
  /// :param data: the typed memory as C array
  ...
  /// Create buffer referencing std::vector with some length without
  /// copying
  /// :param data: the vector to be referenced. If this vector is changed,
...
/// Allocate a bitmap buffer from a memory pool
/// no guarantee on values is provided.
///
/// :param length: size in bits of bitmap to allocate
...
///
/// :param buffers: to be concatenated
/// :param pool: memory pool to allocate the new buffer from
ARROW_EXPORT
