...
class ARROW_EXPORT RecordBatch {
 public:
  ...
  /// :param num_rows: length of fields in the record batch. Each array
  /// should have the same length as num_rows
  /// :param columns: the record batch fields as vector of arrays
  ...
  /// the given schema.
  ///
  /// :param schema: the schema of the empty RecordBatch
  ...
  ///
  /// :param null_to_nan: if true, convert nulls to NaN
  /// :param row_major: if true, create row-major Tensor else column-major Tensor
  /// :param pool: the memory pool to allocate the tensor buffer
  ...
  ///
  /// :param array: the source array, must be a StructArray
  /// :param pool: the memory pool to allocate new validity bitmaps
  ///
  ...
  ///
  /// :param other: the RecordBatch to compare with
  /// :param check_metadata: if true, check that Schema metadata is the same
  /// :param opts: the options for equality comparisons
  ...
  /// Replace the schema with another schema with the same types, but potentially
  ...
  /// :param i: field index, does not boundscheck
  ...
  ///
  /// :param i: field index, which will be boundschecked
  ...
  /// Add new nullable column to the record batch, producing a new
  /// RecordBatch.
  ///
  /// For non-nullable columns, use the Field-based version of this method.
  ///
  /// :param i: field index, which will be boundschecked
  /// :param field_name: name of field to be added
  /// :param column: column to be added
  virtual Result<std::shared_ptr<RecordBatch>> AddColumn(
  ...
  ///
  /// :param i: field index, does boundscheck
  ...
  /// :param offset: the starting offset to slice, through end of batch
  /// :return: new record batch
  virtual std::shared_ptr<RecordBatch> Slice(int64_t offset) const;

  /// Slice each of the arrays in the record batch
  /// :param offset: the starting offset to slice
  /// :param length: the number of elements to slice from offset
  /// :return: new record batch
...
class ARROW_EXPORT RecordBatchReader {
 public:
  ...
  /// Read the next record batch in the stream. Return null for batch
  ...
  /// ```
  ///
  /// :param batch[out]: the next loaded batch, null at end of stream. Returning
  ...
  ///
  /// :param batches: the vector of RecordBatch to read from
  /// :param schema: schema to conform to. Will be inferred from the first
  ///            element if not provided.
  /// :param device_type: the type of device that the batches are allocated on
  ...
  ///
  /// :param batches: an iterator of RecordBatch to read from.
  /// :param schema: schema that each record batch in iterator will conform to.
  /// :param device_type: the type of device that the batches are allocated on
...
/// as count(*)).
///
/// :param batches: a vector of record batches to be concatenated
