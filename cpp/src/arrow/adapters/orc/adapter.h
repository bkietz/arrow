...
/// Information about an ORC stripe
struct StripeInformation {
  /// Offset of the stripe from the start of the file, in bytes
  int64_t offset;
  /// Length of the stripe, in bytes
  int64_t length;
  /// Number of rows in the stripe
  int64_t num_rows;
  /// Index of the first row of the stripe
...
class ARROW_EXPORT ORCFileReader {
 public:
  ...
  /// :param pool: a MemoryPool to use for buffer allocations
  ...
  /// The table will be composed of one record batch per stripe.
  ///
  /// :param include_indices: the selected field indices to read
  /// :return: the returned Table
  ...
  /// The table will be composed of one record batch per stripe.
  ///
  /// :param include_names: the selected field names to read
  /// :return: the returned Table
  ...
  ///
  /// :param row_number: the rows number to seek
  ...
  /// file.
  ///
  /// :param batch_size: the maximum number of rows in each record batch
  ...
  ///
  /// :param batch_size: the maximum number of rows in each record batch
  /// :param include_names: the selected field names to read, if not empty
...
class ARROW_EXPORT ORCFileWriter {
 public:
  ...
  ///
  /// :param output_stream: a pointer to the io::OutputStream to write into
  /// :param write_options: the ORC writer options for Arrow
  ...
  /// Write a table. This can be called multiple times.
  ///
  /// Tables passed in subsequent calls must match the schema of the table that was
  /// written first.
  ///
  /// :param table: the Arrow table from which data is extracted.
  /// :return: Status
  Status Write(const Table& table);

  /// Write a RecordBatch. This can be called multiple times.
  ///
  /// RecordBatches passed in subsequent calls must match the schema of the
  /// RecordBatch that was written first.
  ///
  /// :param record_batch: the Arrow RecordBatch from which data is extracted.
  /// :return: Status
